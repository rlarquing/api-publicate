...
import { BusinessService } from '../../core/service';
import { RolType } from '../../shared/enum';
import {
  BadRequestDto,
  BuscarDto, CreateBusinessDto,
  FiltroGenericoDto,
  ListadoDto, ReadBusinessDto,
  ReadRolDto,
  ResponseDto,
  SelectDto, UpdateBusinessDto, UpdateMultipleBusinessDto,
} from '../../shared/dto';
...
export class BusinessController extends GenericController<BusinessEntity> {
  constructor(
    protected businessService: BusinessService,
    protected configService: ConfigService,
  ) {
    super(businessService, configService, 'business');
  }
  ...
  async findAll(
    ...
    const header: string[] = ['id', 'Nombre', 'Dirección del negocio', 'Usuario'];
    const key: string[] = ['id', 'name', 'addressBusiness', 'user'];
  ...
  @ApiResponse({ status: 403, description: 'Sin autorizacion al recurso.' })
  @ApiResponse({ status: 500, description: 'Error interno del servidor.' })
  async findByIds(@Body() ids: string[]): Promise<ReadBusinessDto[]> {
  ...
  async create(
    ...
    return await super.create(user, createBusinessDto);
  }
  @Post('/multiple')
  @Roles(RolType.ADMINISTRADOR)
  ...
  async createMultiple(
    ...
    return await super.createMultiple(user, createBusinessDto);
  }
  ...
  async import(
    ...
    return await super.import(user, createBusinessDto);
  }
  @Patch('/:id')
  @Roles(RolType.ADMINISTRADOR)
  ...
  async update(
    ...
    return await super.update(user, id, updateBusinessDto);
  }
  ...
  @ApiBody({
    ...
    type: [UpdateMultipleBusinessDto],
  })
  @ApiResponse({
    status: 201,
  ...
  async updateMultiple(
    @GetUser() user: UserEntity,
    @Body() updateMultipleBusinessDto: UpdateMultipleBusinessDto[],
  ): Promise<ResponseDto> {
    return await super.updateMultiple(user, updateMultipleBusinessDto);
  }
